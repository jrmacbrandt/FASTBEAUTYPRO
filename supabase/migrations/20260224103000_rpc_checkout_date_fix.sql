-- ================================================================
-- ⚡ RPC: process_checkout_v2 (Atomic Checkout with Stock Depletion)
-- Data: 2026-02-24
-- Objetivo: Pagamento + CRM + Fidelidade + Baixa de Estoque Integral + Atualização do finalized_at
-- ================================================================
...
BEGIN
    ...
    -- ── 3. Marcar Order como PAGA e atualizar o finalized_at ──
    -- 🛡️ [BLINDADO] Fix para o Dashboard Real-time (atualizar a data de fechamento no momento do pagamento)
            ...
            IF v_stamps >= v_loyalty_target THEN
                ...
                IF v_reward_type IS NOT NULL THEN
                    INSERT INTO public.loyalty_rewards (
                        tenant_id, client_phone, reward_type, reward_reference_id, status, expires_at
                    ) VALUES (
                        p_tenant_id, v_client_phone, v_reward_type, v_reward_ref_id, 'available', NOW() + INTERVAL '30 days'
                    );
                END IF;
            END IF;
        END IF;
    ...
    -- ── 7. PROCESSAMENTO DE ESTOQUE (PRODUTOS VENDIDOS) ────────
