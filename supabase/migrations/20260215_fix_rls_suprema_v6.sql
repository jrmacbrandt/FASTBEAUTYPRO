-- ================================================================
-- 🛡️ SCRIPT DE SEGURANÇA SUPREMA V6.0 (RECUPERAÇÃO TOTAL MASTER)
-- ================================================================
-- Finalidade: Restaurar visibilidade total de dados para o Master,
-- eliminando loops de recursão RLS e usando verificações robustas.
-- Versão Defensiva: Ignora tabelas inexistentes.
-- ================================================================
...
-- 3. DEFINIÇÃO DE POLÍTICAS POR TABELA (MODO DEFENSIVO & UNIVERSAL)
-- ================================================================

-- --- PROFILES (Acesso a todos os perfis para o Master) ---
...
-- --- TENANTS (Acesso a todos os estabelecimentos para o Master) ---
ALTER TABLE tenants ENABLE ROW LEVEL SECURITY;
CREATE POLICY "tenants_universal_access" ON tenants FOR ALL TO authenticated
USING (public.is_master() OR id = public.get_my_tenant_id())
WITH CHECK (public.is_master());

-- --- APPOINTMENTS ---
ALTER TABLE appointments ENABLE ROW LEVEL SECURITY;
CREATE POLICY "appointments_universal_access" ON appointments FOR ALL TO authenticated
USING (public.is_master() OR tenant_id = public.get_my_tenant_id())
WITH CHECK (public.is_master() OR tenant_id = public.get_my_tenant_id());

-- --- SERVICES ---
ALTER TABLE services ENABLE ROW LEVEL SECURITY;
CREATE POLICY "services_universal_access" ON services FOR ALL TO authenticated
USING (public.is_master() OR tenant_id = public.get_my_tenant_id() OR active = true)
WITH CHECK (public.is_master() OR tenant_id = public.get_my_tenant_id());
...
BEGIN
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'clients') THEN
        ...
        CREATE POLICY "clients_universal_access" ON clients FOR ALL TO authenticated
        USING (public.is_master() OR tenant_id = public.get_my_tenant_id())
        WITH CHECK (public.is_master() OR tenant_id = public.get_my_tenant_id());
    END IF;
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'notifications') THEN
        ...
        USING (public.is_master() OR tenant_id = public.get_my_tenant_id() OR receiver_id = auth.uid())
        WITH CHECK (public.is_master() OR tenant_id = public.get_my_tenant_id() OR receiver_id = auth.uid());
    END IF;

    -- STOCK TRANSACTIONS
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'stock_transactions') THEN
        ALTER TABLE stock_transactions ENABLE ROW LEVEL SECURITY;
        CREATE POLICY "stock_universal_access" ON stock_transactions FOR ALL TO authenticated
        USING (public.is_master() OR tenant_id = public.get_my_tenant_id());
    END IF;
