...
-- Data: 2026-02-23
-- Objetivo: Pagamento + CRM + Cartao + Baixa de Estoque Integral
-- ================================================================
...
BEGIN
    ...
    -- ── 7. Baixa Automática de Estoque Atômica ─────────────────
    IF jsonb_typeof(p_products_json) = 'array' AND jsonb_array_length(p_products_json) > 0 THEN
        FOR v_product IN SELECT * FROM jsonb_array_elements(p_products_json) LOOP
            ...
            IF v_product_id IS NOT NULL AND v_qty > 0 THEN
                ...
                IF FOUND THEN
                    v_new_stock := GREATEST(0, COALESCE(v_current_stock, 0) - v_qty);
                    ...
                    INSERT INTO public.stock_transactions (
                        tenant_id, product_id, type, quantity, reason, created_by
                    ) VALUES (
                        p_tenant_id, v_product_id, 'OUT', v_qty, 'Venda Comanda #' || p_order_id, p_created_by
                    );
                END IF;
            END IF;
        END LOOP;
