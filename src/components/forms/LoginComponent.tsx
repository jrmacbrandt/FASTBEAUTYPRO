...
const LoginComponent: React.FC<LoginProps> = ({ type }) => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 6. Get the profile generated by the handle_new_user trigger
            await new Promise(resolve => setTimeout(resolve, 800)); // wait for trigger and indexing
            const { data: existingProfile } = await supabase.from('profiles').select('tenant_id').eq('id', userId).single();
            const triggerTenantId = existingProfile?.tenant_id;
