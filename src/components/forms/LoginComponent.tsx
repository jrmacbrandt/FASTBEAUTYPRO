...
const LoginComponent: React.FC<LoginProps> = ({ type }) => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 5. Auto Login to ensure RLS policies allow updates to profile and tenant
            const { error: loginError } = await supabase.auth.signInWithPassword({ email, password });
            if (loginError) throw new Error('Erro ao autenticar automaticamente após cadastro.');

            // 6. [RESTORED] Robust Profile Verification (Retry Loop)
            // Sometimes the trigger/indexing has a slight delay. We try up to 3 times.
            let existingProfile = null;
            let triggerTenantId = null;

            console.log('[Register] Iniciando verificação de perfil...');
            for (let attempt = 1; attempt <= 3; attempt++) {
                await new Promise(resolve => setTimeout(resolve, 800 * attempt)); // Exponential backoff (800ms, 1600ms, 2400ms)
                ...
                if (p?.tenant_id) {
                    existingProfile = p;
                    triggerTenantId = p.tenant_id;
                    console.log('[Register] Perfil e Tenant localizados na tentativa', attempt);
                    break;
                }

                if (attempt < 3) console.warn(`[Register] Perfil não encontrado na tentativa ${attempt}. Retentando...`);
            }

            if (triggerTenantId) {
                // 7. Update the auto-generated Tenant with the form data
                const { error: tenantError } = await supabase
                    ...
                    .update({
                        ...
                        address_state: state,
                        active: true,
                        // Fix for V4.0: Approved establishments start as has_paid=false unless coupon is used
                    ...
                    .eq('id', triggerTenantId);

                if (tenantError) console.warn('[Register] Aviso ao atualizar dados do estabelecimento:', tenantError.message);

                // 8. Update the auto-generated Profile
                ...
                if (profileError) console.warn('[Register] Aviso ao atualizar perfil:', profileError.message);
            } else {
                console.error('[Register] Falha crítica: Perfil não foi vinculado ao Tenant pela trigger após 3 tentativas.');
                // We proceed to payment page anyway, but the profile might be "orphan"
            }
