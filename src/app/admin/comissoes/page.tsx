        ...
        <div className="space-y-6 md:space-y-10 animate-in fade-in duration-500 pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="p-8 rounded-[2.5rem] border border-white/5 bg-[#121214] relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:scale-110 transition-transform">
                            <span className="material-symbols-outlined text-8xl" style={{ color: colors.primary }}>payments</span>
                        </div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Total em Comissões (Mês)</p>
                    ...
                    <div className="p-8 rounded-[2.5rem] border border-white/5 bg-[#121214] relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:scale-110 transition-transform">
                            <span className="material-symbols-outlined text-8xl" style={{ color: colors.primary }}>receipt_long</span>
                        </div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Total de Serviços</p>
                        <h3 className="text-4xl font-black text-white italic tracking-tighter">
                            {professionals.reduce((acc, curr) => acc + curr.orderCount, 0)}
                        </h3>
                        <p className="text-[9px] font-bold text-slate-500 uppercase mt-4 tracking-widest">Atendimentos finalizados no período</p>
                    </div>
                </div>
                            ...
                            <table className="w-full border-collapse">
                                <thead>
                                    <tr className="bg-white/[0.02]">
                                        <th className="text-left px-8 py-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Profissional</th>
                                        <th className="text-left px-8 py-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Atendimentos</th>
                                        <th className="text-left px-8 py-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Venda Total</th>
                                        <th className="text-right px-8 py-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Comissão</th>
                                    </tr>
                                </thead>
                                            ...
                                            <tr key={pro.id} className="group hover:bg-white/[0.02] transition-colors">
                                                <td className="px-8 py-6">
                                                    <div className="flex items-center gap-4">
                                                        <div className="size-10 rounded-xl overflow-hidden border border-white/10 group-hover:border-[#f2b90d]/50 transition-all">
                                                            {pro.avatar_url ? (
                                                        ...
                                                        <div>
                                                            <p className="text-white font-black italic uppercase tracking-tight text-sm">{pro.full_name}</p>
                                                            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">{pro.role || 'Profissional'}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-8 py-6 text-white font-bold">{pro.orderCount}</td>
                                                <td className="px-8 py-6 text-slate-400 font-bold text-xs uppercase">R$ {pro.totalSales.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                                                <td className="px-8 py-6 text-right">
                                                    <span className="text-[#f2b90d] font-black italic text-lg">
