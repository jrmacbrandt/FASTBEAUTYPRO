...
export default function MasterComunicadosPage() {
    const [owners, setOwners] = useState<{ id: string, full_name: string, tenant_id: string }[]>([]);
    const [selectedRecipients, setSelectedRecipients] = useState<string[]>([]);
    const [messageText, setMessageText] = useState('');
    const [messageTitle, setMessageTitle] = useState('COMUNICADO FASTBEAUTY');
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            setSelectedRecipients(owners.map(o => o.id));
        } else {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            // No Master Admin, enviamos para múltiplos administradores (Owners)
            // Precisamos do tenant_id de cada um para o RLS do lado do destinatário (se aplicável),
            // mas o sendNotification usa o tenantId passado.
            // Aqui enviamos individualmente para garantir que o receiver_id esteja correto.

            await Promise.all(selectedRecipients.map(recipientId => {
                const owner = owners.find(o => o.id === recipientId);
                return sendNotification(
                    ...
                    'master_info',
                    'high',
                    owner?.tenant_id // Associa ao tenant do dono para que ele possa ver via RLS
                );
            }));

            setStatus('success');
            setMessageText('');
            setMessageTitle('COMUNICADO FASTBEAUTY');
            setSelectedRecipients([]);
            await fetchHistory();
            setTimeout(() => setStatus('idle'), 3000);
        } catch (error: any) {
            console.error('Erro ao enviar comunicado master:', error);
    ...
    const handleDeleteMessage = async (id: string) => {
        if (!confirm('Deseja excluir permanentemente este comunicado global?')) return;
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 relative pb-20 px-4 md:px-0 lg:pt-10">
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none p-6 bg-black/20 backdrop-blur-sm">
                    <div className="bg-emerald-500 text-white px-8 md:px-10 py-6 md:py-8 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl animate-in zoom-in duration-300 flex flex-col items-center gap-3 md:gap-4 pointer-events-auto">
                        ...
                        <div className="text-center">
                            <h4 className="text-lg md:text-xl font-black italic uppercase">Transmitido</h4>
                            <p className="text-white/80 text-[8px] md:text-[10px] font-bold uppercase tracking-widest mt-1">Comunicado Global enviado!</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <header className="mb-4">
                ...
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.2em] italic opacity-60">Master Admin Control Panel</p>
            </header>
                ...
                <div className="bg-[#121214] p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] border border-white/5 shadow-2xl animate-in fade-in slide-in-from-right-4 duration-300">
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-10 gap-4">
                        <div className="flex items-center gap-3 md:gap-4">
                            ...
                            <div>
                                <h3 className="text-xl md:text-2xl font-black italic uppercase text-white tracking-tight leading-none mb-1">Nova Transmissão</h3>
                                <p className="text-slate-500 text-[8px] md:text-[10px] font-bold uppercase tracking-widest italic opacity-60">Envio direto para administradores de unidade</p>
                            </div>
                        </div>
                        <button
                    ...
                    <div className="space-y-4 md:space-y-8">
                        {/* Destinatários (Admins/Owners) */}
                        <div className="space-y-4">
                            <div className="flex items-center justify-between px-2">
                                <label className="text-[9px] md:text-[10px] font-black uppercase text-slate-500 tracking-widest italic opacity-60">Administradores Selecionados ({selectedRecipients.length})</label>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                                {loading ? (
                                    <div className="col-span-full py-10 flex flex-col items-center opacity-30">
                                        ...
                                        <p className="text-[10px] font-black uppercase tracking-widest">Carregando admins...</p>
                                    </div>
                                ) : owners.map(owner => (
                                    <label
                                        key={owner.id}
                                        className={`flex items-center gap-3 p-4 rounded-2xl border transition-all cursor-pointer group ${selectedRecipients.includes(owner.id) ? 'bg-[#f2b90d]/10 border-[#f2b90d]/30' : 'bg-black/40 border-white/5 hover:border-white/10'}`}
                                        onClick={() => handleToggleRecipient(owner.id)}
                                    >
                                        <div className="relative">
                                            <div className={`size-4 rounded-md border-2 flex items-center justify-center transition-all ${selectedRecipients.includes(owner.id) ? 'bg-[#f2b90d] border-[#f2b90d]' : 'bg-black border-white/10 group-hover:border-white/20'}`}>
                                                {selectedRecipients.includes(owner.id) && <span className="material-symbols-outlined text-black text-[12px] font-black">check</span>}
                                            </div>
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <p className={`font-black uppercase italic tracking-tight text-[11px] truncate ${selectedRecipients.includes(owner.id) ? 'text-white' : 'text-slate-400 group-hover:text-slate-300'}`}>
                                                {owner.full_name}
                                            </p>
                                        </div>
                                    </label>
                        ...
                        <div className="space-y-1.5 md:space-y-2">
                            <label className="text-[9px] md:text-[10px] font-black uppercase text-slate-500 ml-2 tracking-widest italic opacity-60">Título da Transmissão</label>
                            <input
                                type="text"
                                placeholder="Ex: MANUTENÇÃO, ATUALIZAÇÃO, PROMOÇÃO..."
                                value={messageTitle}
                        ...
                        <div className="space-y-1.5 md:space-y-2">
                            <label className="text-[9px] md:text-[10px] font-black uppercase text-slate-500 ml-2 tracking-widest italic opacity-60">Conteúdo do Comunicado</label>
                            <textarea
                                rows={6}
                                placeholder="Explique os detalhes para os administradores das unidades..."
                                value={messageText}
                        ...
                        >
                            {status === 'sending' ? 'TRANSMITINDO...' : 'ENVIAR PARA SELECIONADOS'}
                        </button>
                    </div>
                </div>
            ) : (
                <div className="bg-[#121214] p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] border border-white/5 shadow-2xl animate-in fade-in slide-in-from-left-4 duration-300">
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-12 gap-6">
                        <div className="flex items-center gap-3 md:gap-4">
                            ...
                            <div>
                                <h3 className="text-xl md:text-2xl font-black italic uppercase text-white tracking-tight leading-none mb-1">Log de Transmissões</h3>
                                <p className="text-slate-500 text-[8px] md:text-[10px] font-bold uppercase tracking-widest italic opacity-60">Controle de mensagens enviadas aos parceiros</p>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4 max-h-[600px] overflow-y-auto custom-scrollbar pr-2">
                        {historyLoading ? (
                            <div className="py-24 flex flex-col items-center opacity-30">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-widest">Lendo logs globais...</p>
                            </div>
                        ) : history.length === 0 ? (
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center gap-4 md:gap-6 min-w-0">
                                                <div className="size-10 md:size-14 bg-white/5 rounded-xl md:rounded-2xl flex items-center justify-center group-hover:bg-[#f2b90d]/10 group-hover:text-[#f2b90d] transition-all shrink-0">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-[8px] md:text-[10px] font-bold text-slate-500 uppercase tracking-widest truncate">{count} {count === 1 ? 'Transmissão' : 'Transmissões'}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-3 md:gap-4 shrink-0">
                                                {selectedCount > 0 && (
                                                    <div className="bg-[#f2b90d] text-black px-2 md:px-3 py-1 rounded-full text-[7px] md:text-[10px] font-black uppercase italic whitespace-nowrap">
                                                        {selectedCount} <span className="hidden xs:inline">selecionada{selectedCount > 1 ? 's' : ''}</span>
                                                    </div>
                                                )}
                                                <span className="material-symbols-outlined text-slate-600 group-hover:text-[#f2b90d] group-hover:translate-x-1 transition-all text-xl md:text-2xl">chevron_right</span>
                                            </div>
                                        </div>
                                    );
                                })}
